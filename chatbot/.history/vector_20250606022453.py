...
def extract_ward(address):
    # Hàm giả định để trích xuất ward từ địa chỉ
    return address.split(",")[0].strip()

def extract_district(address):
    # Hàm giả định để trích xuất district từ địa chỉ
    return address.split(",")[1].strip() if len(address.split(",")) > 1 else ""

def clean_text(text):
    # Hàm giả định để làm sạch văn bản
