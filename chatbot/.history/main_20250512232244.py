...
def haversine(lat1, lon1, lat2, lon2):
    # Tính khoảng cách giữa 2 điểm theo vĩ độ, kinh độ (đơn vị: km)
...
async def ask_question(req: QuestionRequest):
    ...
    # Nếu có phòng phù hợp, lấy tọa độ phòng đầu tiên để tìm các phòng gần nhất
    nearby_text = ""
    if not filtered_rooms.empty:
        filtered_rooms = filtered_rooms.head(3)
        main_room = filtered_rooms.iloc[0]
        lat1, lon1 = float(main_room["latitude"]), float(main_room["longitude"])
        # Loại bỏ phòng chính khỏi danh sách all_rooms
        others = all_rooms[all_rooms["title"] != main_room["title"]].copy()
        # Tính khoảng cách
        others["distance"] = haversine(lat1, lon1, others["latitude"].astype(float), others["longitude"].astype(float))
        # Lấy 2 phòng gần nhất
        nearby = others.nsmallest(2, "distance")
        if not nearby.empty:
            nearby_text = "\nĐề xuất phòng gần đó:\n"
            for idx, row in nearby.iterrows():
                nearby_text += f"- {row['title']}: {row['address']} (cách khoảng {row['distance']:.2f} km)\n"
