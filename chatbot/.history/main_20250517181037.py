...
Nếu có, hãy liệt kê đầy đủ tên, địa chỉ, giá và mô tả của từng phòng cho người dùng.
"""
...
async def process_query(question: str):
    ...
    if ward:
        ward_clean = clean_text(ward)  # "Thượng Đình" -> "thuong dinh"
        filtered_rooms = all_rooms[
            (all_rooms["address_clean"].str.contains(ward_clean, na=False)) &
            (all_rooms["status"].isin(["ROOM_RENT", "ENABLE"]))
        ]
    ...
    if ward:
        ward_clean = clean_text("nghĩa đô")  # clean_text sẽ chuyển thành "nghia do"
        filtered_rooms = all_rooms[all_rooms["address_clean"].str.contains(ward_clean, na=False)]
    elif district:
