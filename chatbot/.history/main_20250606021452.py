...
def create_vector_db():
    ...
    if add_documents:
        ...
        for i, row in tqdm(df.iterrows(), total=len(df)):
            # Log metadata để kiểm tra
            logger.info(f"Document {i}:")
            logger.info(f"  ward_clean: {row.get('ward_clean')}")
            logger.info(f"  district_clean: {row.get('district_clean')}")
            metadata = {
                "title": row["title"],
                "address": row["address"],
                "ward": row["ward"],
                ...
                "ward_clean": clean_text(row["ward"]),  # Đảm bảo có trường này
                "district_clean": clean_text(row["district"]),  # Đảm bảo có trường này
            }
            page_content = (
                f"Tên: {row['title']}\n"
...
async def process_query(question: str):
    ...
    metadata_filter = {}
    if district:
        metadata_filter = {"district_clean": clean_text(district)}
    elif ward:
        metadata_filter = {"ward_clean": clean_text(ward)}
    else:
        metadata_filter = {}
    ...
    docs = retriever.invoke(
        ...
        search_kwargs={
            ...
            **search_kwargs
        } if metadata_filter else search_kwargs
    )
