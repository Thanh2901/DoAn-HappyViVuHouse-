...
def extract_location(text, keyword):
    ...
    # Regex linh hoạt hơn cho các từ khóa vị trí
    patterns = [
        rf"{keyword}\s+([a-zA-ZÀ-ỹ\s]+)",  # Mẫu ban đầu
        rf"muốn\s+tìm\s+phòng\s+(?:ở|o|ỏe)\s+{keyword}\s+([a-zA-ZÀ-ỹ\s]+)",  # Xử lý "muốn tìm phòng ở phường X"
        rf"{keyword}\s+([a-zA-ZÀ-ỹ\s]+)(?:\s+có\s+phòng|phòng\s+trọ)",  # Xử lý "phường X có phòng"
        rf"(?:ở|o|ỏe)\s+{keyword}\s+([a-zA-ZÀ-ỹ\s]+)"  # Xử lý "ở phường X"
    ]
...
def guess_district_or_ward(text, all_rooms):
    text = text.lower()
    # Tạo tập hợp tất cả quận, phường để tìm kiếm nhanh
    districts = set()
    wards = set()
...
async def process_query(question: str):
    ...
    if "xung quanh" in question_lower and m:
        ...
        if not main_room.empty:
            ...
            # Tính toán khoảng cách vector hóa
    ...
    if filtered_rooms.empty:
        ...
        # Giới hạn số lượng kết quả để tăng tốc độ
