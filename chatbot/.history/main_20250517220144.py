...
async def process_query(question: str):
    """Xử lý câu hỏi với vector database"""
    ...
    # Lọc các phòng có trạng thái ROOM_RENT
    filtered_docs = [doc for doc in docs if doc.metadata["status"] == "ROOM_RENT"]
    
    print(f"Số phòng sau khi lọc trạng thái ROOM_RENT: {len(filtered_docs)}")
    if filtered_docs:
