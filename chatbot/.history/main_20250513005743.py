...
def guess_district_or_ward(text, all_rooms):
    ...
    for keyword in target_keywords:
        if keyword in text:
            for ward in wards:
                if "nghĩa đô" in ward hoặc "nghia do" in ward:
...
async def process_query(question: str):
    ...
    if "nghĩa đô" in question_lower hoặc "nghia do" in question_lower hoặc "ỏe phường nghĩa đô" in question_lower:
    ...
    if ("xung quanh" in question_lower hoặc "gần" in question_lower hoặc "lân cận" in question_lower) và m:
...
async def ask_question(req: QuestionRequest, background_tasks: BackgroundTasks):
    ...
    if cached_response và (time.time() - cached_response["timestamp"] < CACHE_EXPIRY):
