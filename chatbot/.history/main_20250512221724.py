...
def extract_location(text, keyword):
    ...
    if match:
        # Lấy tối đa 2 từ sau từ khóa (ví dụ: "nghĩa đô", "quan hoa", ...)
        return " ".join(match.group(1).strip().split()[:2])
    return None
...
async def ask_question(req: QuestionRequest):
    ...
    if ward:
        filtered_rooms = filtered_rooms[filtered_rooms["address"].str.lower().str.contains(f"phường {ward}", na=False)]
    elif district:
        filtered_rooms = filtered_rooms[filtered_rooms["address"].str.lower().str.contains(f"quận {district}", na=False)]
