...
Nếu danh sách trên KHÔNG có phòng nào, hãy trả lời lịch sự rằng chưa có dữ liệu phù hợp.
Nếu có, hãy LIỆT KÊ lại thông tin các phòng đó cho khách hàng.
...
def clean_text(text):
    # Đưa hàm lên vị trí cao hơn trong file để có thể được dùng ở mọi nơi
...
exclude_titles = []
filtered_rooms = all_rooms[~all_rooms["title"].isin(exclude_titles)]

print(filtered_rooms[filtered_rooms["address"].str.lower().str.contains("quận cầu giấy", na=False)])
...
async def ask_question(req: QuestionRequest):
    ...
    if "xung quanh" in question_lower and m:
        ...
        # Sử dụng hàm clean_text từ phạm vi ngoài
        main_room = all_rooms[all_rooms["title"].apply(lambda x: clean_text(x) == clean_text(room_title))]
        print("DEBUG - room_title:", room_title)
        print("DEBUG - all_titles:", all_rooms["title"].tolist())
        if not main_room.empty:
            lat1, lon1 = float(main_room.iloc[0]["latitude"]), float(main_room.iloc[0]["longitude"])
            others = all_rooms[all_rooms["title"] != room_title].copy()
            others["distance"] = others.apply(
                lambda row: haversine(lat1, lon1, float(row["latitude"]), float(row["longitude"])), 
                axis=1
            )
    ...
    if filtered_rooms.empty:
        ...
        # Loại bỏ các phòng không muốn hiển thị
        exclude_titles = []
        filtered_rooms = filtered_rooms[~filtered_rooms["title"].isin(exclude_titles)]
        # Nếu vẫn muốn giới hạn số lượng, dùng head(n)
        # filtered_rooms = filtered_rooms.head(3)  # Bỏ dòng này nếu muốn lấy tất cả
        reviews_text = ""
        for idx, row in filtered_rooms.iterrows():
            reviews_text += f"- Tên phòng: {row['title']}\n"
