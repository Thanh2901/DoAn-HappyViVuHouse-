...
const isAvailable = (status) => status === "ROOM_RENT" || status === "CHECKED_OUT";
const filteredTableData = tableData.filter(item => {
    if (statusFilter === "ALL") return true;
    if (statusFilter === "RENTED") return !isAvailable(item.status);
    if (statusFilter === "AVAILABLE") return isAvailable(item.status);
    return true;
});
