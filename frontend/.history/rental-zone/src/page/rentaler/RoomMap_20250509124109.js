...
const RoomMap = ({ rooms }) => {
    ...
    useEffect(() => {
        ...
        if (!mapInstanceRef.current) {
            ...
            // Tạo custom pane với zIndex cao cho labels
            const labelsPane = createCustomPane(mapInstanceRef.current, 'labels-pane', 650);
            
            // Thêm base layer (bản đồ nền không có nhãn)
            ...
            // Tuỳ chỉnh style cho bản đồ
            mapInstanceRef.current.getContainer().style.fontSize = '14px';
            
            // Thêm lớp bản đồ Openstreetmap tiếng Việt với font lớn hơn
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                maxZoom: 19,
                className: 'larger-map-labels', // Class để tăng kích thước nhãn
                pane: 'labels-pane',  // Sử dụng pane riêng cho nhãn
                opacity: 1,
            }).addTo(mapInstanceRef.current);
            
            // Tuỳ chỉnh style cho popup toàn cục
            const originalInitialize = L.Popup.prototype.initialize;
            L.Popup.prototype.initialize = function (options, source) {
                options = options || {};
                // Tăng kích thước font mặc định cho tất cả popup
                options.className = (options.className || '') + ' large-popup';
                // Tăng chiều rộng mặc định cho popup
                options.minWidth = options.minWidth || 250;
                options.maxWidth = options.maxWidth || 300;
                return originalInitialize.call(this, options, source);
            };
    ...
    useEffect(() => {
        ...
        const greenIcon = L.icon({
            ...
            iconSize: [30, 48],  // Tăng kích thước marker lớn hơn nữa
            iconAnchor: [15, 48],
            popupAnchor: [1, -40],
        ...
        validRooms.forEach((item) => {
            ...
            // Áp dụng các tùy chọn popup để phóng to text
            marker.bindPopup(popupContent, {
                minWidth: 300,  // Tăng kích thước tối thiểu
                maxWidth: 380,  // Tăng kích thước tối đa
                className: "large-popup",
                autoPan: true,
                closeButton: true,
                autoClose: false,  // Ngăn tự động đóng khi click vào popup khác
                closeOnClick: false  // Ngăn đóng khi click vào map
            });
    ...
    return (
        <>
            ...
            <style>
                {`
                /* Tăng kích thước font cho popup và làm đẹp */
                ...
                /* Tăng kích thước cho toàn bộ popup */
                ...
                /* Tăng kích thước cho popup arrow */
                .leaflet-popup-tip {
                    width: 20px !important;
                    height: 20px !important;
                }
                
                /* Style cho các control của leaflet */
                ...
                /* Tăng kích thước font cho toàn bộ labels trên bản đồ */
                .larger-map-labels {
                ...
                /* Tăng kích thước font cho attribution text */
