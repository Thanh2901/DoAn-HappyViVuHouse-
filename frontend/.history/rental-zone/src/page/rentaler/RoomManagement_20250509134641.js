...
function RoomManagement(props) {
    ...
    const filteredTableData = tableData.filter(item => {
        ...
        // Đã thuê: ROOM_RENT hoặc HIRED
        if (statusFilter === "RENTED") return item.status === "ROOM_RENT" || item.status === "HIRED";
        // Chưa thuê: CHECKED_OUT hoặc (nếu muốn giống bảng) ROOM_RENT hoặc CHECKED_OUT
        if (statusFilter === "AVAILABLE") return item.status === "CHECKED_OUT" || item.status === "ROOM_RENT";
        return true;
    });
