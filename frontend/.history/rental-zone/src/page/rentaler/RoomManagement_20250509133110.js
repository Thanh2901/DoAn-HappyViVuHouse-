...
function RoomManagement(props) {
        ...
        const timer = setTimeout(() => {
            ...
            try {
                ...
                L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
                    maxZoom: 19,
                    detectRetina: true // Giúp map rõ nét hơn trên màn hình độ phân giải cao
    ...
    console.log(tableData.map(item => item.status));
    ...
    const filteredTableData = tableData.filter(item => {
        ...
        if (statusFilter === "AVAILABLE") return item.status === "CHECK_OUT";
        return true;
    });
