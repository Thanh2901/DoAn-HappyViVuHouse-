...
function RoomManagement(props) {
    ...
    // Thay thế đoạn useEffect cho bản đồ bằng đoạn code đã sửa
    useEffect(() => {
        ...
        // Nếu không có dữ liệu hoặc phần tử DOM chưa sẵn sàng thì thoát
        if (!tableData || tableData.length === 0 || !mapRef.current) return;
        ...
        // Đảm bảo phần tử DOM đang hiển thị trước khi khởi tạo bản đồ
        // Thêm lệnh setTimeout để đảm bảo DOM đã render hoàn toàn
        setTimeout(() => {
            try {
                // Kiểm tra lại xem phần tử DOM còn tồn tại không
                if (!mapRef.current) return;
                
                // Khởi tạo bản đồ với một thời gian trễ để đảm bảo DOM đã được render
                ...
                // Sử dụng nhiều cách để đảm bảo bản đồ hiển thị đúng kích thước
                // Sử dụng MutationObserver để phát hiện thay đổi về hiển thị
                ...
                // Quan sát thay đổi trên phần tử cha chứa bản đồ
                ...
                // Sử dụng cả setTimeout và requestAnimationFrame để đảm bảo bản đồ được cập nhật kích thước
                setTimeout(() => {
                    if (mapInstanceRef.current) {
                        mapInstanceRef.current.invalidateSize();
                    }
                ...
                // Thêm sự kiện resize để cập nhật kích thước bản đồ khi cửa sổ thay đổi kích thước
                const handleResize = () => {
                    if (mapInstanceRef.current) {
                        mapInstanceRef.current.invalidateSize();
                    }
                ...
                // Nếu có lỗi, đảm bảo dọn dẹp tài nguyên
                if (mapInstanceRef.current) {
                    mapInstanceRef.current.remove();
                    mapInstanceRef.current = null;
                }
            }
        }, 300); // Đợi 300ms trước khi khởi tạo bản đồ
