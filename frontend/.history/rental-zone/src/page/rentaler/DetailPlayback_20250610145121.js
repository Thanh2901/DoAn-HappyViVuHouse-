...
function DetailPlayback(props) {
  ...
  useEffect(() => {
    ...
    getCameraRecords(id)
      .then((data) => {
        console.log("Records data:", data); // Debug dữ liệu
        setRecords(data);
        setLoading(false);
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (selectedRecord && videoRef.current) {
      const videoUrl = selectedRecord.url || selectedRecord.filePath || "";
      const fullUrl = `${minioHost}/${videoUrl}`; // Ghép trực tiếp minioHost với videoUrl
      videoRef.current.src = fullUrl;
      videoRef.current.load();
      videoRef.current
        .play()
        .catch((error) => console.error("Video play failed:", error));
      console.log("Video source set to:", fullUrl);
    }
