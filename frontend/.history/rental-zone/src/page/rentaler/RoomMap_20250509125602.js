...
const RoomMap = ({ rooms }) => {
    ...
    useEffect(() => {
        ...
        if (!mapInstanceRef.current) {
            mapInstanceRef.current = L.map(mapRef.current, {
                ...
                preferCanvas: true,
            }).setView([21.0285, 105.8542], 14); // Tăng zoom từ 12 lên 14
    ...
    useEffect(() => {
        ...
        const greenIcon = L.icon({
            ...
            iconSize: [36, 56], // Tăng từ [30, 48] lên [36, 56]
            iconAnchor: [18, 56],
            popupAnchor: [1, -48],
            shadowUrl: "https://unpkg.com/leaflet@1.7.1/dist/images/marker-shadow.png",
            shadowSize: [60, 60], // Tăng shadow size
        ...
        if (validRooms.length > 1) {
            ...
            map.fitBounds(bounds, { padding: [30, 30], maxZoom: 14 }); // Tăng maxZoom từ 13 lên 14
        } else if (validRooms.length === 1) {
            map.setView([
                parseFloat(validRooms[0].latitude),
                parseFloat(validRooms[0].longitude)
            ], 14); // Tăng zoom từ 12 lên 14
        }
    ...
    return (
        <>
            ...
            <style>
                ...
                .custom-popup h3 {
                    font-size: 26px !important; /* Tăng từ 22px */
                    font-weight: bold;
                    margin: 0 0 12px 0;
                    color: #333;
