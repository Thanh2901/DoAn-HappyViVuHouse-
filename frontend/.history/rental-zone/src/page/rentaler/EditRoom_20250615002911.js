...
function EditRoom(props) {
  ...
  const [originalRoomType, setOriginalRoomType] = useState('REGULAR');
  ...
  useEffect(() => {
    getRoom(id)
      .then(response => {
        ...
        setOriginalRoomType(room.type || "REGULAR");
      })
      .catch(error => {
  ...
  const handleSubmit = (event) => {
    ...
    RoomService.updateRoom(id, formData)
      .then(response => {
        if (response.status === 400 && response.data && response.data.message === "You don't have permission to update the room type") {
          toast.error(response.data.message);
        } else {
          toast.success(response.data.message || translate("rentaler:rental_rooms_management:update_success"));
          navigate('/rentaler/room-management');
        }
      })
      .catch(error => {
        console.error("Backend error:", error.response ? error.response.data : error);
        if (error.response && error.response.status === 400 && error.response.data && error.response.data.message === "You don't have permission to update the room type") {
