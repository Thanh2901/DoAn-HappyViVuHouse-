...
function AddRoom(props) {
  ...
  const handleAssetChange = (event, index) => {
    ...
    setRoomData(prevState => ({
      ...
      assets: prevState.assets.map((asset, i) =>
        i === index ? { ...asset, [name]: name === 'number' ? value : value } : asset
      )
    }));
  ...
  const handleSubmit = (event) => {
    ...
    RoomService.addNewRoom(formData)
      .then(response => {
        toast.success(response.message || translate("rentaler:rental_rooms_management:post_success"));
        setRoomData({
          title: '',
          description: '',
          price: 0,
      ...
      .catch(error => {
        toast.error(error.message || translate("rentaler:rental_rooms_management:post_error"));
      })
      .finally(() => {
        setLoading(false);
      });
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-buttons">
                    <button
                      ...
                      className="btn btn-primary"
                    >
                      {loading ? translate("rentaler:rental_rooms_management:posting") : translate("rentaler:rental_rooms_management:post_new_feed")}
                    </button>
                    <button
                      type="button"
