...
const Chat = ({ authenticated, currentUser, onLogout, onNewMessage }) => {
  ...
  // Log currentUser at component mount
  useEffect(() => {
    console.log('Chat initialized with currentUser:', currentUser);
  }, [currentUser]);
      ...
      return new Promise((resolve, reject) => {
        ...
        const onConnect = () => {
          ...
          subscriptionRef.current = client.subscribe('/topic/public', (message) => {
            ...
            if (!receivedMessage.createdBy || !currentUser.name) {
              console.warn('Missing createdBy or currentUser.name:', {
                createdBy: receivedMessage.createdBy,
                currentUserName: currentUser.name
              });
            }

            const messageId = receivedMessage.id || `${receivedMessage.type}_${receivedMessage.createdBy || 'unknown'}_${receivedMessage.sentAt}`;
            const messageWithId = {
              ...receivedMessage,
            ...
            if (messageWithId.type === 'JOIN' || messageWithId.type === 'LEAVE') {
              ...
              setSystemNotifications(prev => {
                const newNotifications = [...prev, messageWithId];
                console.log('Updated system notifications:', newNotifications);
        ...
        tempClient.connect({}, () => {
          historySubscriptionRef.current = tempClient.subscribe('/topic/public/history', (message) => {
            ...
            const processedMessages = historyMessages.map(msg => {
              const isOwn = msg.createdBy?.toLowerCase() === currentUser?.name?.toLowerCase();
              ...
              return {
                ...
                isOwnMessage: isOwn,
                uniqueId: msg.id || `${msg.type}_${msg.createdBy || 'unknown'}_${msg.sentAt}`
              };
            });
  ...
  const sendMessage = (content) => {
    ...
    try {
      ...
      setMessages(prev => {
        const newMessages = [...prev, { ...chatMessage, isOwnMessage: true, uniqueId: chatMessage.clientId }];
        console.log('Optimistically updated messages:', newMessages);
        return newMessages;
      });
    } catch (error) {
