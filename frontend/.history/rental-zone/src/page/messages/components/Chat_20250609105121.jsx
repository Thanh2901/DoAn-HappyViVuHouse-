...
const Chat = ({ authenticated, currentUser, onLogout, onNewMessage }) => {
    ...
    // Thêm Set để track các notification đã xử lý
    const processedNotificationsRef = useRef(new Set());
    const processedMessagesRef = useRef(new Set()); // Track messages đã xử lý

    // Enhanced focus/blur handling with auto-reconnect
    useEffect(() => {
        const handleFocus = async () => {
            ...
            // Tự động kết nối lại nếu đang disconnect và có currentUser
        ...
        const handleBlur = () => {
            ...
            // Clear any pending reconnect timeouts when window loses focus
    ...
    // Enhanced reconnect function with retry mechanism
                ...
                const onConnect = () => {
                    ...
                    if (!subscriptionRef.current) {
                        subscriptionRef.current = client.subscribe('/topic/public', (message) => {
                            ...
                            // Tạo unique ID cho message để tránh duplicate
                            ...
                            if (messageWithId.type === 'JOIN' || messageWithId.type === 'LEAVE') {
                                ...
                                setSystemNotifications(prev => {
                                    // Double check để tránh duplicate trong state
    ...
    // Monitor connection status and auto-reconnect when disconnected (only when window is focused)
    useEffect(() => {
        ...
        const checkConnection = () => {
            // Only auto-reconnect if window is focused and not already reconnecting
        ...
        // Check connection every 3 seconds when window is focused
        if (isWindowFocused) {
    ...
    // Reset message count và clear processed sets khi component mount hoặc currentUser thay đổi
    useEffect(() => {
        if (currentUser) {
            console.log('👤 Current user changed, resetting message count to 0');
            dispatch(resetMessageCount());
            // Clear processed sets khi user thay đổi
            ...
            await new Promise((resolve, reject) => {
                tempClient.connect({}, () => {
                    ...
                    historySubscriptionRef.current = tempClient.subscribe('/topic/public/history', (message) => {
                        ...
                        // Clear processed sets và populate lại với history data
                        processedMessagesRef.current.clear();
                        processedNotificationsRef.current.clear();
                        
                        // Add history messages/notifications to processed sets
    ...
    const cleanupConnection = () => {
        ...
        // Clear processed messages/notifications sets khi cleanup
        processedMessagesRef.current.clear();
        processedNotificationsRef.current.clear();
    };

    // Original reconnect function (now used for manual reconnection)
    const reconnect = () => {
    ...
    useEffect(() => {
        ...
        const handleBeforeUnload = (e) => {
            autoReconnectOnFocusRef.current = false; // Disable auto-reconnect when leaving
