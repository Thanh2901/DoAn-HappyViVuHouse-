...
const Chat = () => {
  ...
  // Thêm hàm để load lịch sử chat
  const loadChatHistory = async (client) => {
    try {
      client.send('/app/chat/public/history', {}, {});
      
      // Đăng ký một subscription tạm thời để nhận lịch sử
      const historySubscription = client.subscribe('/topic/public/history', (response) => {
        const history = JSON.parse(response.body);
        console.log('Received chat history:', history);
        
        // Chuyển đổi tin nhắn lịch sử sang định dạng phù hợp
        const formattedHistory = history.map(msg => ({
          ...msg,
          clientId: `hist_${msg.id}_${Date.now()}`,
  ...
  useEffect(() => {
    ...
    const onConnect = () => {
      ...
      // Load lịch sử chat ngay sau khi kết nối
