...
const Chat = ({ authenticated, currentUser, onLogout, onNewMessage }) => {
                ...
                subscriptionRef.current = client.subscribe('/topic/public', (message) => {
                    ...
                    if (messageWithId.type === 'JOIN' || messageWithId.type === 'LEAVE') {
                        ...
                        setMessages(prev => {
                            ...
                            // CHỈ đếm tin nhắn từ người khác (không phải tin nhắn của mình)
                            if (messageWithId.type === 'CHAT' && !messageWithId.isOwnMessage) {
                                dispatch(incrementMessageCount());
                                console.log(`New message from ${messageWithId.createdBy}, incrementing count`);
                            }
    ...
    // Thêm useEffect để log khi newMessageCount thay đổi
    useEffect(() => {
        console.log(`Total new messages count updated: ${newMessageCount}`);
    }, [newMessageCount]);
    ...
    // Thêm function để clear message count khi user click vào chat
    ...
    return (
        <div className="h-70 d-flex flex-column" onClick={handleChatClick}>
            <div className="card flex-grow-1">
                <div className="card-header d-flex justify-content-between align-items-center py-2">
                    <h4 className="mb-0 fw-bold">PUBLIC CHAT ROOM</h4>
                    <div className="d-flex align-items-center">
                        {/* Hiển thị số lượng tin nhắn mới từ Redux với animation */}
                        {newMessageCount > 0 && (
