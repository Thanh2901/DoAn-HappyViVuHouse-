...
const Chat = ({ authenticated, currentUser, onLogout, onNewMessage }) => {
          ...
          subscriptionRef.current = client.subscribe('/topic/public', (message) => {
            ...
            if (messageWithId.type === 'JOIN' || messageWithId.type === 'LEAVE') {
              ...
              if (processedMessagesRef.current.has(messageId)) {
                console.log('🔕 Duplicate message ignored:', messageId); // Debug log
                return;
              }
              processedMessagesRef.current.add(messageId);
              setMessages(prev => {
                const newMessages = [...prev, messageWithId];
                console.log('Updated messages state:', newMessages); // Debug log
                return newMessages;
              });
      ...
      await new Promise((resolve, reject) => {
        tempClient.connect({}, () => {
          historySubscriptionRef.current = tempClient.subscribe('/topic/public/history', (message) => {
            ...
            console.log('📜 Chat history:', historyMessages); // Debug log
  ...
  const sendMessage = (content) => {
    if (!connected || !stompClientRef.current || !currentUser) {
      console.error('Cannot send message: Invalid state', { connected, hasStompClient: !!stompClientRef.current, currentUser }); // Debug log
      throw new Error('WebSocket not connected or user not authenticated');
    }
    if (!content.trim()) {
      console.log('Empty message ignored'); // Debug log
      return;
    }
    const chatMessage = {
      content,
