...
const Message = ({ message, isOwnMessage, username }) => {
  const renderFile = () => {
    ...
    if (!message.fileName) {
      console.warn('Missing fileName in FILE message:', message);
  ...
  return (
    <div className={`d-flex flex-column ${isOwnMessage ? 'align-items-end' : 'align-items-start'} mb-2`}>
      ...
      >
        {/* FIXED: Chỉ hiển thị tên người gửi cho tin nhắn KHÔNG PHẢI của mình */}
        {!isOwnMessage && (
          <small className={`d-block fw-bold mb-1 ${isOwnMessage ? 'text-white-50' : 'text-primary'}`}>
            {message.createdBy || username || 'Unknown'}
          </small>
        )}
        ...
        {/* Thời gian */}
        <small className={`d-block text-${isOwnMessage ? 'white-50' : 'muted'} text-end mt-1`}>
