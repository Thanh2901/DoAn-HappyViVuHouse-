...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    // Tắt log của Stomp bằng cách sử dụng hàm rỗng thay vì null
    client.debug = () => {}; // Đây là cách đúng để tắt debug messages

    const onConnect = () => {
      console.log(`WebSocket connected on client: ${currentUser.name}`);
      setConnected(true);
      ...
      if (!subscriptionRef.current) {
        subscriptionRef.current = client.subscribe('/topic/public', (message) => {
          ...
          console.log('Subscription active on client:', currentUser.name);
    ...
    const onError = (error) => {
      console.error(`WebSocket connection error on client: ${currentUser.name}`, error);
      setConnected(false);
