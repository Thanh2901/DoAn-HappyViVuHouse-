...
const Input = ({ sendMessage, disabled, reconnect }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if ((trimmedMessage || selectedFiles.length > 0) && !disabled) {
      // Send text message
      if (trimmedMessage) {
        console.log('Attempting to send text message:', trimmedMessage); // Debug log
        try {
      ...
      console.log('Submit blocked: empty message and no files, or disabled:', { trimmedMessage, disabled }); // Debug log
    }
  };
  ...
  const handleInputClick = () => {
    if (disabled) {
      console.log('Input clicked while disabled, triggering reconnect'); // Debug log
      reconnect();
    }
  };
