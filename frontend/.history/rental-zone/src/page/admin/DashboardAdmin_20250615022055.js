...
const DashboardAdmin = (props) => {
  ...
  const revenueData = financialStats
    ? [
        financialStats.totalRevenue || 0,
        financialStats.totalAdminFee || 0,
      ]
    : [0, 0];
  const statusCounts = financialStats
    ? financialStats.statusCounts.map((item) => item.count) || [0, 0, 0]
    : [0, 0, 0];
  const statusLabels = financialStats
    ? financialStats.statusCounts.map((item) => item.status) || [
        "PENDING",
        "SUCCESS",
        "FAILED",
      ]
