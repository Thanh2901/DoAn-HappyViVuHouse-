...
// Lấy frame ảnh (JPEG) của camera
export function getCameraFrame(id) {
  return fetch(API_BASE_URL + `/cameras/${id}/frame`, {
    method: "GET",
    headers: {
...
// Lấy stream camera (trả về response, bạn cần xử lý stream ở FE)
export function streamCamera(id) {
  return fetch(API_BASE_URL + `/cameras/${id}/stream`, {
    method: "GET",
    headers: {
      Authorization: "Bearer " + localStorage.getItem(ACCESS_TOKEN),
    },
